...
public interface UserRepository extends JpaRepository<UserEntity, String> {
  ...
  // - findBy: {SELECT * WHERE}: 필드명을 기준으로 모든 컬럼을 조회, findBy 바로 뒤에 필드명을 붙여서 작성
  // 필드명은 반드시 첫글자가 대문자로 작성되어야 함
  // SELECT *FROM user WHERE = user_name=?
  // List 타입으로 반환받는 경우 결과가 없으면 길이가 0인 리스트를 반환
  List<UserEntity> findByUserName(String userName);
  
  // existsBy: 조건에 해당하는 레코드가 존재하는지 여부 확인
  // boolean 형태로 반환
  ...
  // And / Or: and연산 혹은 or 연산에 사용됨, 필드와 필드 사이 작성
  // And Or 연산 우선순위 주의
  ...
  List<UserEntity> getUserList(@Param("name") String userName, @Param("telNumber") String userTelNumber);
  ...
  @Query(
    ...
    "AND user_tel_number = :userTelNumber ", nativeQuery=true)
